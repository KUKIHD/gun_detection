...
Используется **детекция + трекинг (ByteTrack)**: каждому объекту присваивается стабильный ID (`person #3`, `gun #1`), рамки плавно следуют за объектом между кадрами без дёргания.
...
Отключить трекинг (только детекция по кадрам):

```bash
python detect_video.py --video video/detected.mp4 --no-track
```
...
Демонстрация на `video/detected.mp4` (1920×1080, 30 FPS, 1286 кадров) с **трекингом ByteTrack**:
...
| Время обработки | 40.1 с |
| Средний FPS | 32.0 |
| Уникальных треков gun | 9 |
| Уникальных треков person | 41 |

На выходном видео:

- цветные bounding box с **ID трека** (`person #3`, `gun #1`) — объект следует между кадрами без дёргания;
- счётчики активных объектов и уникальных треков на каждом кадре.
...
| Этап | Описание |
|------|----------|
| Курсовая (текущий) | Детекция gun + person, трекинг ByteTrack, YOLOv8n |
